...
//         // ✅ UPDATED: Also update payment status when marking as delivered
//         const updateFields = {
//             orderStatus: status
//         };

//         // If marking as delivered, also set payment status to paid
//         if (status === 'delivered') {
...
//             },
//             {
//                 $set: updateFields  // ✅ Use dynamic update fields
//             },
//             { new: true }
//         );
...
//         // ✅ KEEP YOUR EXISTING REVENUE LOGIC (it's correct)
//         const vendor = await Vendor.findById(req.vendor.id);
        
//         if (vendor) {
//             // Add to revenue when order is delivered and payment is successful
//             if (status === 'delivered' && 
//                 (order.paymentStatus === 'paid' || order.paymentStatus === 'completed' || 
//                  order.paymentMethod === 'UPI')) {
                
//                 vendor.totalRevenue = (vendor.totalRevenue || 0) + order.totalAmount;
//                 vendor.platformFees = (vendor.platformFees || 0) + (order.totalAmount * 0.05);
                
//                 console.log(`✅ Added revenue: ${order.totalAmount} for order ${orderId}`);
//                 await vendor.save();
//             }
            
//             // Deduct revenue if order is cancelled
//             if (status === 'cancelled') {
//                 vendor.totalRevenue = (vendor.totalRevenue || 0) - order.totalAmount;
//                 vendor.platformFees = (vendor.platformFees || 0) - (order.totalAmount * 0.05);
                
//                 console.log(`❌ Removed revenue: ${order.totalAmount} for cancelled order ${orderId}`);
//                 await vendor.save();
//             }
//         }
...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (vendor) {
            ...
            // Deduct revenue if order is cancelled
        // ✅ FIXED - Only deduct for truly cancelled orders, NOT returns
// ✅ FIXED - Only deduct revenue for truly cancelled orders
if (status === 'cancelled' && !status.includes('return')) {
    vendor.totalRevenue = (vendor.totalRevenue || 0) - order.totalAmount;
    vendor.platformFees = (vendor.platformFees || 0) - (order.totalAmount * 0.05);
...
// ✅ Don't touch revenue for any return statuses
console.log(`🔍 Order status updated to: ${status} - Revenue unchanged`);
